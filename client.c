...
void insert_entry_text (GtkButton *button, Widgets *w)
{
	GtkTextBuffer *buffer;
	GtkTextMark *mark;
	GtkTextIter iter;
...
gboolean check_player(gpointer data)
{
	...
	if (FD_ISSET(sockfd, &read_fds)) {
		if((numbytes = recv(sockfd, message_recv, MSG_LEN, 0)) <= 0) {
			perror("recv");
			crt_end_menu(CONNECTION_ERR);
			return FALSE;
		} else {
			id = message_recv[1];
			printf("Game id %d\n", id);
			message_send[0] = CHECK;
			if (send(sockfd, message_send, MSG_LEN, 0) == -1)
				perror("send");
	...
	GtkWidget *label;
 	...
 	if(connect_to_server() != 0) {
		...
		if(strlen(name) == 0 || strlen(name) > 20) {
			crt_end_menu(INVALID_NAME);
		} else {
			memset(message_send, 0, MSG_LEN);

			if(side == 1)
				message_send[0] = CREATE_O;
			else
				message_send[0] = CREATE_X;
			course = 1;
...
void callback_choice_game (GtkWidget *widget, gpointer data) {
	...
	if(message_recv[0] == BUSY) {
		crt_end_menu(BUSY);
	} else {
		if(message_recv[0] == CREATE_O)
			side = 2;
		else
			side = 1;
		exit_flag = 0;
...
void callback_join_game (GtkWidget *widget, gpointer data)
{
	if(connect_to_server() != 0) {
		...
		if (message_recv[0] == GET_GAMES_LIST) {
